...
public class PrometeoCarController : MonoBehaviour
{
    ...
    void Update()
    {
      ...
      */
      if (useTouchControls && touchControlsSetup){
        ...
        if((!_inputBackward && !_inputForward)){
          ThrottleOff();
        }
        if((!_inputBackward && !_inputForward) && !_inputHandbrake && !deceleratingCar){
          InvokeRepeating("DecelerateCar", 0f, 0.1f);
          deceleratingCar = true;
        }
        if(!_inputLeft && !_inputRight && steeringAxis != 0f){
          ResetSteeringAngle();
        }
  ...
  private bool _inputBackward = false;
  public void Input_Backward (InputAction.CallbackContext cxt) { _inputBackward = cxt.performed; }
  private bool _inputLeft = false;
  public void Input_Left (InputAction.CallbackContext cxt) { _inputLeft = cxt.performed; }
  private bool _inputRight = false;
  public void Input_Right (InputAction.CallbackContext cxt) { _inputRight = cxt.performed; }
  private bool _inputHandbrake = false;
  public void Input_Handbrake(InputAction.CallbackContext cxt)
  {
    _inputHandbrake = cxt.performed;
    if (cxt.canceled)
      RecoverTraction();
  }
}
